...
document.addEventListener('DOMContentLoaded', () => {
        ...
        link.addEventListener('click', e => {
            if (link.hash) {
                ...
                if (targetElement) {
                    ...
                    document.querySelectorAll(`header nav a[href="${link.hash}"], #mobile-menu a[href="${link.hash}"]`).forEach(activeLink => activeLink.classList.add('active'));
    ...
    window.addEventListener('scroll', () => {
        ...
        if (sections.length > 0 && window.pageYOffset < sections[0].offsetTop - headerOffset - 10) {
            ...
            document.querySelectorAll(`header nav a[href="#hero"], #mobile-menu a[href="#hero"]`).forEach(heroLink => heroLink.classList.add('active'));
        }
    });
    ...
    function generateSchedule(startDate, initialCoins, processes, locale, currentTranslations) {
        ...
        for (let i = 1; i <= processes; i++) {
            ...
            schedule.push({
                date: date.toLocaleDateString(locales[locale] || 'en-US', { day: '2-digit', month: '2-digit', year: 'numeric' }),
                day: date.toLocaleDateString(locales[locale] || 'en-US', { weekday: 'long' }),
                process: i,
                coins: new Intl.NumberFormat(locales[locale] || 'en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(coins)
            });
    ...
    async function fetchLivePrices() {
        ...
        if (burniPriceElement) burniPriceElement.textContent = new Intl.NumberFormat(currentLocale, { style: 'currency', currency: 'USD', minimumFractionDigits: 6, maximumFractionDigits: 8 }).format(prices.burni.priceUSD);
        if (circulatingSupplyElement) circulatingSupplyElement.textContent = `${new Intl.NumberFormat(currentLocale, {maximumFractionDigits: 1}).format(prices.burni.circulatingSupply / 1000)}K`;
        if (holdersElement) holdersElement.textContent = new Intl.NumberFormat(currentLocale).format(prices.burni.holders);
        if (trustlinesElement) trustlinesElement.textContent = new Intl.NumberFormat(currentLocale).format(prices.burni.trustlines);

        const xrpPriceElement = document.getElementById('xrpPriceValue');
        if (xrpPriceElement) xrpPriceElement.textContent = new Intl.NumberFormat(currentLocale, { style: 'currency', currency: 'USD', minimumFractionDigits: 4 }).format(prices.xrp.priceUSD);

        const xpmPriceElement = document.getElementById('xpmPriceValue');
        if (xpmPriceElement) xpmPriceElement.textContent = new Intl.NumberFormat(currentLocale, { style: 'currency', currency: 'USD', minimumFractionDigits: 10, maximumFractionDigits: 12 }).format(prices.xpm.priceUSD);

        const lastUpdatedTimestampElement = document.getElementById('lastUpdatedTimestamp');
        if (lastUpdatedTimestampElement) lastUpdatedTimestampElement.textContent = new Date().toLocaleString(currentLocale);

        if (supplyChartInstance && translations[currentLang] && prices.burni.circulatingSupply !== undefined) { 
            supplyChartInstance.data.datasets[0].data = [prices.burni.circulatingSupply, 1000000 - prices.burni.circulatingSupply];
            const supplyLabelKey = translations[currentLang].kpi_circulating_supply || translations.en.kpi_circulating_supply;
            const maxSupplyLabelKey = translations[currentLang].kpi_max_supply || translations.en.kpi_max_supply;
                        ...
                        tooltip: {
                            callbacks: {
                                label: context => {
                                    ...
                                    if (label) label += ': ';
                                    if (context.parsed !== null) label += new Intl.NumberFormat(locales[currentLang]).format(context.parsed) + ' Tokens';
                                    return label;
                                }
                            }
                        }
        ...
        if (athAtlCtx) {
            athAtlChartInstance = new Chart(athAtlCtx, {
                ...
                options: {
                    ...
                    indexAxis: 'y',
                    scales: {
                        x: { beginAtZero: true, title: { display: true, text: 'Price in XRP', color: '#374151' }, ticks: { color: '#374151', callback: value => new Intl.NumberFormat(locales[currentLang]).format(value) } },
                    ...
                    plugins: {
                        ...
                        tooltip: {
                            callbacks: {
                                label: context => context.dataset.label + ': ' + new Intl.NumberFormat(locales[currentLang]).format(context.parsed.x) + ' XRP'
                            }
                        }
                    }
                }
    ...
    try {
        ...
        if (scheduleCtx) {
            ...
            const chartLabels = scheduleData.map(row => row.date).slice(0,4).concat(['...']).concat(scheduleData.map(row => row.date).slice(-3));
            const chartDatasetData = scheduleData.map(row => parseFloat(row.coins)).slice(0,4).concat([null]).concat(scheduleData.map(row => parseFloat(row.coins)).slice(-3));
            ...
            scheduleChartInstance = new Chart(scheduleCtx, {
                ...
                options: {
                    ...
                    maintainAspectRatio: false,
                    scales: {
                        y: { beginAtZero: false, title: { display: true, text: 'Number of Coins', color: '#374151' }, ticks: { color: '#374151', callback: value => new Intl.NumberFormat(locales[currentLang]).format(value) } }, 
                        ...
                        tooltip: {
                            callbacks: {
                                label: context => {
                                    ...
                                    return value !== null ? label + ': ' + new Intl.NumberFormat(locales[currentLang]).format(value) : '';
                                }
                            }
                        }
                    }
    ...
    setTimeout(() => {
        ...
        if (twitterTimeline && (twitterTimeline.offsetHeight === 0 || twitterTimeline.offsetWidth === 0) && !twitterTimeline.querySelector('iframe')) {
    ...
    const translations = {
        'en': { /* ... Alle englischen Übersetzungen ... */ },
        'de': { /* ... Alle deutschen Übersetzungen ... */ },
        'es': { /* ... Alle spanischen Übersetzungen ... */ },
        'fr': { /* ... Alle französischen Übersetzungen ... */ },
        'ar': { /* ... Alle arabischen Übersetzungen ... */ },
        'bn': { /* ... Alle bengalischen Übersetzungen ... */ },
        'ja': { /* ... Alle japanischen Übersetzungen ... */ },
        'pt': { /* ... Alle portugiesischen Übersetzungen ... */ },
        'ko': { /* ... Alle koreanischen Übersetzungen ... */ },
        'ru': { /* ... Alle russischen Übersetzungen ... */ },
        'tr': { /* ... Alle türkischen Übersetzungen ... */ },
        'zh': { /* ... Alle chinesischen Übersetzungen ... */ },
        'hi': { /* ... Alle Hindi Übersetzungen ... */ },
        'it': { /* ... Alle italienischen Übersetzungen ... */ }
    };
    ...
    function setLanguage(lang) {
        ...
        elementsToTranslate.forEach(element => {
            ...
            let translationText = effectiveTranslations[key] || translations['en'][key] || `Missing: ${key}`; 
        ...
        if (supplyChartInstance && effectiveTranslations.kpi_circulating_supply && effectiveTranslations.kpi_max_supply) {
            const supplyValue = (typeof prices !== 'undefined' && prices && prices.burni && prices.burni.circulatingSupply) ? prices.burni.circulatingSupply : fallbackPrices.burni.circulatingSupply;
        ...
        if (scheduleChartInstance && effectiveTranslations.schedule_chart_label && effectiveTranslations.schedule_chart_y_axis && effectiveTranslations.schedule_chart_x_axis) {
            scheduleChartInstance.data.datasets[0].label = effectiveTranslations.schedule_chart_label;
            scheduleChartInstance.options.scales.y.title.text = effectiveTranslations.schedule_chart_y_axis;
            scheduleChartInstance.options.scales.x.title.text = effectiveTranslations.schedule_chart_x_axis;
        ...
        faqSearch.addEventListener('input', () => {
            ...
            faqSearchTimeout = setTimeout(() => {
                ...
                document.querySelectorAll('#faq .section-card').forEach(card => {
                    ...
                    card.style.display = (question.includes(query) || answer.includes(query)) ? 'block' : 'none';
                });
            }, 300); 
        });
    }
